...
- [x] **No export of a SQL result set.** Distinct from exporting the dataset: `/api/query` truncates `data` to 100 rows, so a client-side export of the results table would silently drop everything past row 100. This needs a query-scoped backend endpoint (run the query, stream the full frame) rather than a frontend change. Not in scope for this pass, but don't ship a "download results" button without it.
  - **Shipped.** Backend: `POST /api/query/export` (`app/api/query_export.py`) re-runs the query through the same sandboxed `run_readonly_query()` the SQL tab uses, but with a 200,000-row cap instead of the 5,000-row one `/api/query` applies for its JSON response — not unbounded, since a result-materializing endpoint with no ceiling is a memory/DoS seam regardless of who's allowed to call it, but far past what the 100-row display table ever showed. `safe_sql.run_readonly_query()` gained a `row_limit` parameter (default unchanged) to share that sandboxing code rather than duplicating it. Returns sanitized CSV (`to_safe_csv`, same formula-injection guard as dataset export) as an attachment, with an `X-Result-Truncated` header (now in `expose_headers`) so the frontend can tell the user if the cap was hit. 7 new endpoint tests in `tests/test_query_export.py`. Frontend: `lib/api/export-query.ts` + a "Download results" button in `sql-tab.tsx`'s results header, exporting the query that actually produced what's on screen (tracked in a ref, not whatever's currently typed), with a toast on truncation or failure. Verified in the browser with a 250-row dataset: the table showed "Showing first 100 of 250 rows", and Download Results returned all 250 via `/api/query/export` (confirmed 200 over the network panel).
...
- [x] **Inline merge — deliberately split from detection**
  ...
  - **Shipped.** Backend: `POST /api/transform/rename` (`app/api/transform.py`) takes `{dataset_id, column, new_name}`, validates the column exists and the new name doesn't collide, and mutates the stored frame in place via a new `dataset_manager.rename_column()` under the existing `DATASETS` lock. 7 endpoint tests in `tests/test_transform.py`, including one that re-queries via `/api/query` afterward to prove the rename is visible outside the endpoint that made it, not just echoed back. Frontend: `lib/api/rename-column.ts` is the thin caller predicted above; each row in Compare Tab's "Suggested Matches" now has "Use "X"" / "Use "Y"" buttons that rename the *other* dataset's column to the chosen name, update `app/page.tsx`'s dataset state from the response (`handleColumnRenamed`), and toast on success/failure. Verified end to end in the browser: renamed `customer_name` → `Customer Name` in one dataset, watched it move from "Unique" to "Common Columns" live, then confirmed via a fresh SQL query against that dataset that the backend frame — and its autocomplete schema — actually changed.
...
- [x] **Surface a "repair report" after upload**, e.g. in `FileUploadModal` or as a follow-up toast/panel:
  ...
  - **Shipped.** Backend: `app/core/csv_repair.py` implements the full detect → tokenize → reconcile → normalize pipeline described below and returns a `repair_report` object (`clean`, `encoding`, `encoding_confidence`, `delimiter`, `warnings[]`, `stats{}`) from `/api/upload` for `.csv` files. Frontend: `FileUploadModal` parses it onto `Dataset.repairReport` (camelCased, see `parseRepairReport`) and fires a `sonner` toast per file summarizing the warnings; `app/layout.tsx` now mounts `<Toaster />`, which wasn't wired up anywhere before this. 48 backend unit tests (`tests/test_csv_repair.py`) plus 7 endpoint-level tests (`tests/test_upload_endpoint.py`) cover the edge-case checklist below.
- [x] **"Repair preview" (nice-to-have, high trust value)**: a side-by-side of a handful of original-vs-fixed rows so users can verify what changed before relying on the dataset. Only worth doing once the backend can return sample before/after rows — and it should be fetched on demand rather than stuffed into the upload response, for the same sessionStorage-size reason.
  - **Shipped.** Backend: `repair_csv()` snapshots up to 5 `{before, after}` row pairs at the one point in the pipeline where both are row-index-aligned — `reconciled_rows` (post structural-repair, pre value-normalization) becomes the DataFrame's rows 1:1 and in order, since `_reconcile_rows` pads/merges every kept row to exactly `len(header)`. Kept on `RepairReport.preview`, deliberately excluded from `to_dict()`/the upload response (per the sessionStorage-size reasoning above) and only rows where at least one cell actually changed are kept. `dataset_manager.create_dataset()` takes an optional `repair_preview` and stores it alongside the frame; a new `GET /api/dataset/{id}/repair-preview` (`app/api/repair_preview.py`) serves it on demand. 7 tests in `tests/test_repair_preview.py`. Frontend: `lib/api/repair-preview.ts` + a `RepairPreviewDialog` in `file-toolbar.tsx`, reached via a "View before/after rows" link inside the existing repair-warning hover card — fetches lazily on first open, shows a before/after table with changed cells colored red/green. Verified in the browser with a file containing whitespace, `NULL`, and `N/A` cells: the dialog correctly showed `" 30 "` → `30.0`, `NULL` → `null`, `N/A` → `null`, with unchanged cells (names) left uncolored.
- [x] **Distinguish "repaired with warnings" from "clean upload"** in the `FileToolbar` dataset chips (e.g. a small warning-triangle badge), so users know which files to double-check.
  - **Shipped.** An amber wrench icon appears in the chip's action row (next to rename/download/remove) whenever `repairReport.clean` is false; hovering it opens a `HoverCard` listing every warning in full. Deliberately visually distinct from the existing yellow `AlertTriangle` used for "stale, no longer on server" (Section 2), since they mean different things and can both apply.
- [x] Treat this as a dependency for **Section 5 (JSON/XLSX upload)** — the same detect → repair → normalize pipeline should apply regardless of source format — and as the guarantee that **Section 1's export** returns repaired, normalized data rather than the raw bytes as parsed.
  - JSON/XLSX still go through plain `pd.read_json`/`pd.read_excel` (no `repair_report`) — pandas' own readers are already fault-tolerant for those formats in the ways that matter (encoding, structure), and the roadmap's own sequencing note (bottom of this file) only asks that Section 6 land *before* Section 5, not that Section 5 be rewritten now. Section 1's export already reads back whatever `df` is stored server-side, which is now the repaired/normalized frame for CSV uploads — no export-side change needed.

### Known limitations (not fixable, or deliberately out of scope for this pass)

- **Streaming for very large files.** Not implemented — the whole file is still read into memory and tokenized in one pass. The existing 20MB upload cap and 5M-cell post-parse guard (Section 5) bound the damage, but a true streaming repair pass is future work if those caps are ever raised.
- **Silent data loss from spreadsheet mangling** (values already turned into scientific notation, or already stripped of leading zeros, *before* the file reached us) can be detected and flagged, but the lost digits/zeros cannot be reconstructed — the report says so explicitly rather than pretending to fix it.
- **Genuinely ambiguous conventions** — day-first vs. month-first dates, decimal-comma vs. thousands-comma with no other signal in the column, 2-digit-year century — are left as original text (or use a named, flagged default) rather than guessed, per this section's guiding principle. A user who knows their own data's convention gets no forced auto-fix to fight.
- **CSV-injection sanitization is a narrower net than a blanket rule.** Cells starting with `=` or `@` are always neutralized; `+`/`-` are only neutralized when not immediately followed by a digit, to avoid corrupting ordinary signed numbers stored as text. A payload shaped like `-2+3+cmd|'/C calc'!A1` (leading digit right after the sign) would not be caught by this narrower rule. Full protection would need sanitization at the point data is opened in a spreadsheet tool, not just at ingest.
- **An unescaped delimiter inside a value that happens not to change the row's field count** (rare, but possible) can't be detected or fixed — the overflow-merge repair only fires when the row's column count no longer matches the header.
